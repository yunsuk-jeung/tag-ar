...
- 다양한 기종에서의 검증: 현재 iPhone 16 Pro에서만 테스트했으며, 특히 LiDAR가 없는 기종에서는 depth refinement 없이 동작이 검증되지 않았습니다.
- 더 많은 파라미터 튜닝: 현재 `config/tracker.json` 값은 제한된 환경에서 맞춘 것으로, 기종/조명/태그 크기 등 다양한 조건에 대한 추가 튜닝이 필요합니다.
- dynamic object motion 모델 기반 보간: 태그의 속도/가속도 등 motion 모델을 추정해, 트래킹이 끊긴 구간에서도 pose를 예측·보간하여 증강 객체가 끊기지 않고 유지되도록 확장.
- ATW/ASW 기반 지연 보간: tracking 완료 시점과 실제 이미지가 렌더링되는 시점 사이의 딜레이를, Asynchronous Time Warp / Asynchronous Space Warp 기법으로 보간하여 motion-to-photon latency로 인한 cube 흔들림/지연 감소.
